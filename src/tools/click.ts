  ...
  return [
    {
      name: "browser_click",
      description:
        "Click element by CSS, XPath, text, role, or ref (from compact map). Auto-fallback: if Playwright click fails on visibility, retries with JS click internally. Accepts ref='e5' from compact_map — uses cached stable locator (data-tsid, aria-label, role+name).",
      inputSchema: {
        type: "object" as const,
        properties: {
          selector: { type: "string", description: "CSS, XPath, text selector, or ref ID (e.g. 'e5')" },
          button: { type: "string", enum: ["left", "right", "middle"], default: "left" },
          click_count: { type: "number", default: 1 },
          force: { type: "boolean", default: false, description: "Force JS click even if element is invisible/overlayed" },
    ...
    },
    {
      ...
      description:
        "Force-click element via JS — bypasses visibility, overlays, and pointer-event interception.",
      inputSchema: {
        type: "object" as const,
        properties: {
    ...
    },
    {
      name: "browser_fill",
      description:
        "Fill input field. Works with contenteditable divs via execCommand + input event dispatch.",
      inputSchema: {
        type: "object" as const,
        properties: {
          selector: { type: "string" },
...
export async function handleClick(args: unknown, state: ServerState, page: any) {
  ...
  try {
    ...
    if (e.message?.includes("not visible") || e.message?.includes("intercepts pointer events")) {
      await page.evaluate((sel: string) => {
        const el = document.querySelector(sel);
        if (el) (el as HTMLElement).click();
      }, resolved);
